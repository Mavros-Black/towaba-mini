...
export async function POST(
  ...
  try {
    ...
    if (!permissions?.delete_campaigns && adminRole.role !== 'super_admin') {
      return NextResponse.json(
        { error: 'Access denied: Insufficient permissions to delete campaigns' },
        { status: 403 }
      )
    }
    ...
    if (deleteError) {
      console.error('Admin delete campaign error:', deleteError)
      return NextResponse.json(
    ...
    return NextResponse.json({
      success: true,
      message: 'Campaign deleted successfully by admin override',
      data: result
    })
